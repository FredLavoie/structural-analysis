export function validateForm(obj) {
    ...
    for (let i = 0; i < joints.length; i += 5) {
        ...
        if (
            isNotBetweenZeroAndOne(joints[i + 2]) ||
            isNotBetweenZeroAndOne(joints[i + 3]) ||
            isNotBetweenZeroAndOne(joints[i + 4])
        ) {
    ...
    for (let i = 0; i < members.length; i += 5) {
        if (
            isNotIntegerNumber(members[i]) ||
            isNotIntegerNumber(members[i + 1]) ||
            members[i] > numJoints ||
            members[i + 1] > numJoints
        ) {
    ...
    for (let i = 0; i < numJointLoads; i++) {
        ...
        for (let i = 0; i < endIndex; i += 4) {
            ...
            if (isNotAnyTypeOfNumber(loads[i + 1])) {
                (valid = false),
                    messages.push(
                        "One or more joint x-direction force values are incorrect in the joint load input section",
                    );
            }
            ...
            if (isNotAnyTypeOfNumber(loads[i + 3])) {
                (valid = false),
                    messages.push(
                        "One or more joint moment force values are incorrect in the joint load input section",
                    );
            }
        }
    }
    ...
    for (let i = 0; i < numMemLoads; i++) {
        ...
        for (let i = startIndex; i < loads.length; i += 4) {
            ...
            if (isNotAnyNegativeZeroNumber(loads[i + 1])) {
                (valid = false),
                    messages.push(
                        "One or more point load distance values are incorrect in the member load input section",
                    );
            }
            if (isNotAnyTypeOfNumber(loads[i + 2])) {
                (valid = false),
